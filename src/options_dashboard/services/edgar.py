...
class EdgarDownloader:
    ...
    def download(self, request: EdgarDownloadRequest) -> EdgarDownloadResponse:
        ...
        filings_dir = edgar_root / "filings"
        manifests_dir = edgar_root / "manifests"
        for directory in (metadata_dir, submissions_dir, exports_dir, filings_dir, manifests_dir):
    ...
    def _should_skip_download(self, destination: Path, manifest: dict[str, Any], edgar_root: Path) -> bool:
        if not destination.exists():
            return False
        relative_name = destination.relative_to(edgar_root).as_posix()
    ...
    def _update_manifest_entry(
        ...
        *,
        ...
        relative_name = file_path.relative_to(edgar_root).as_posix()
        files[relative_name] = {
            "checksum": checksum,
